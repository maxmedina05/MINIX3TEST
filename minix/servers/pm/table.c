...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_PRINTDATE) 	= do_printdate,
	CALL(PM_PRINTPROC) 	= do_printproc,
	CALL(PM_SEMDOWN) 	= do_semdown,
	CALL(PM_SEMUP) 		= do_semup,
	CALL(PM_SEMCREATE)	= do_semcreate,
	CALL(PM_SEMDEL)		= do_semdel
};
